...
def score_lead(payload: dict[str, Any]) -> LeadScore:
    ...
    return LeadScore(score, "hot" if score >= 75 else "warm" if score >= 45 else "cold")
...
def entropy_score(payload: dict[str, Any], rejected: bool) -> float:
    ...
    return round(max(-1.0, min(2.5, o_miss + l_admin + v_op - 1.0)), 3)


def recommend_action(score_band: str, urgency: str | None) -> str:
    if normalize(urgency) in {"emergency", "asap"}:
...
def estimate_revenue(payload: dict[str, Any], score: LeadScore, rejected: bool) -> int:
    ...
    return int(base * {"hot": 1.1, "warm": 0.7, "cold": 0.35}[score.band])


def classify_lead(payload: dict[str, Any]) -> tuple[bool, str | None]:
    suburb = normalize(payload.get("suburb"))
    allowed = {normalize(s) for s in SITE_CONFIG.get("service_suburbs", [])}
    if suburb and allowed and suburb not in allowed:
...
def write_lead(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    rejected, reason = classify_lead(payload)
    ent = entropy_score(payload, rejected)
    lead_id = str(uuid.uuid4())
    now = utc_now_iso()
    status = "rejected" if rejected else ("qualified" if score.band in {"hot", "warm"} else "nurture")
    action = recommend_action(score.band, payload.get("urgency"))
    revenue = estimate_revenue(payload, score, rejected)
    trust_due = (datetime.now(timezone.utc) + timedelta(hours=24)).isoformat() if score.band == "hot" and not rejected else None
    ...
    with sqlite3.connect(DB_PATH) as conn:
        conn.execute(
            ...
            """,
            (
                lead_id, now, now, payload["name"], payload.get("business_name"), payload["email"], payload.get("phone"),
                payload.get("suburb"), payload["service_type"], payload.get("urgency"), payload.get("budget_aud"), payload.get("notes"),
                score.value, score.band, status, "new_lead", action, revenue, ent, reason, trust_due,
            ),
        )

    insert_event("lead_created", payload, lead_id)
    insert_event("automation_triggered", {
        "lead_id": lead_id,
...
class ServiceHandler(BaseHTTPRequestHandler):
    server_version = "SAWebWorksFLNA/2.1"

    def _read_json(self) -> dict[str, Any]:
        l = int(self.headers.get("Content-Length", "0"))
        return {} if l <= 0 else json.loads(self.rfile.read(l).decode("utf-8"))
    ...
    def do_GET(self) -> None:  # noqa: N802
        if self.path in {"/", "/ui"}:
            if UI_PATH.exists():
                self._write_html(HTTPStatus.OK, UI_PATH.read_text(encoding="utf-8"))
            else:
                self._write_html(HTTPStatus.OK, "<h1>UI not found</h1>")
            return
        ...
        if self.path == "/api/leads":
            with sqlite3.connect(DB_PATH) as conn:
                ...
                rows = conn.execute(
                    "SELECT id, created_at, name, email, suburb, service_type, score, score_band, status, pipeline_state, estimated_revenue_aud, entropy_score, rejected_reason FROM leads ORDER BY created_at DESC LIMIT 100"
                ).fetchall()
        ...
        if self.path == "/api/revenue-summary":
            with sqlite3.connect(DB_PATH) as conn:
                conn.row_factory = sqlite3.Row
                totals = conn.execute(
                    "SELECT COUNT(*) AS total_leads, COALESCE(SUM(estimated_revenue_aud),0) AS projected_revenue_aud, COALESCE(SUM(CASE WHEN score_band='hot' THEN 1 ELSE 0 END),0) AS hot_leads, ROUND(COALESCE(AVG(entropy_score),0),3) AS avg_entropy_score FROM leads"
                ).fetchone()
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        if missing:
            ...
            return
        self._write_json(HTTPStatus.CREATED, write_lead(payload))
